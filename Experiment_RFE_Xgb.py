...
#param_grid = dict(RFE__n_features_to_select=[2,3,4,5,6,7,8,9,10], classifier__learning_rate=[0.001, 0.05, 0.1, 10],  classifier__max_depth=[10], classifier__n_estimators=[100])
#param_grid = dict(RFE__n_features_to_select=[10], classifier__max_depth=[2, 3, 4, 5], classifier__n_estimators=[100, 200])
...
for train_index, test_index in kf.split(X_df):
    ...
    class_weights = compute_class_weight("balanced", np.unique(y_train), y_train[target])
    cw = class_weights[0]/class_weights[1]

    # Create the classifier object
    selected_classifier = "XgBoost"
    classifier = xgb.XGBClassifier(scale_pos_weight=cw)
    ...
    selected_features = [g for g, x in zip(all_features, estimator.named_steps['RFE'].support_) if x]
